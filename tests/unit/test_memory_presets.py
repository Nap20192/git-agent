...
def test_priority_explicit_over_env_over_default(monkeypatch):
    ...
    assert resolve_memory_preset("full_history").name == "full_history"  # явный > env
    assert resolve_memory_preset().name == "aggressive"  # env > дефолт
    monkeypatch.delenv("GIT_AGENT_MEMORY_PRESET")
    assert resolve_memory_preset().name == PRODUCTION_MEMORY_PRESET  # дефолт
...
def test_default_depends_on_model_provider():
    ...
    # провайдер вне long-context allowlist → фолбэк-продакшен пресет
...
def test_unknown_name_raises_not_falls_back(monkeypatch):
    ...
    monkeypatch.setenv("GIT_AGENT_MEMORY_PRESET", "martian")  # и из env тоже
    with pytest.raises(ValueError, match="Unknown memory preset"):
        resolve_memory_preset()
