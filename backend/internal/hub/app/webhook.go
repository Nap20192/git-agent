...
func (s *WebhookService) Handle(ctx context.Context, provider string, repoID int64, auth domain.WebhookAuth, e domain.Event, body []byte) {
	...
	if err != nil {
		zap.S().Errorw("webhook: repository lookup failed", "repositoryId", repoID, "err", err)
		return
	}
	if repo == nil {
		zap.S().Infow("webhook: dropped, unknown repository", "repositoryId", repoID, "provider", provider)
		return
	}
	secret, err := s.Secrets.Decrypt(repo.WebhookSecretEnc)
	if err != nil {
		zap.S().Warnw("webhook: dropped, no usable secret", "repositoryId", repoID)
		return
	}
	if !domain.VerifyWebhook(provider, string(secret), body, auth) {
		zap.S().Warnw("webhook: dropped, invalid signature", "repositoryId", repoID, "provider", provider)
		return
	}

	duplicate, instanceIDs, err := s.FanOut(ctx, repo, e, body)
	if err != nil {
		zap.S().Errorw("webhook: ingest failed", "repositoryId", repoID, "deliveryId", e.DeliveryID, "err", err)
		return
	}
	if duplicate {
		zap.S().Infow("webhook: duplicate delivery", "provider", provider, "deliveryId", e.DeliveryID)
	} else if len(instanceIDs) == 0 {
		zap.S().Infow("webhook: no matching subscriptions, journaled only",
