...
Сейчас конфигурится один сервер — cve-mcp-server (stdio, поднимается через
`uv run --project <path> python -m cve_mcp.server`). Любая ошибка старта/загрузки
деградирует в предупреждение и пустой список: недоступный MCP не роняет Ран
(fail-open для доступности — в отличие от fail-closed бинда схем в core/tools).
"""
