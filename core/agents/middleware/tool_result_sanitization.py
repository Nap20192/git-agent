...
Наша модель угроз — вредоносный РЕПОЗИТОРИЙ: его содержимое приходит модели
через sandbox_run/read_file сырым. Файл с <report_contract>/<system-reminder>
внутри — prompt-инъекция в наших же доверенных тегах. Стратегия референса:
de-identify, don't reject — тег экранируется и читается как литерал.

Локальный вывод доверенных тулов не трогается (не манглить код/логи) —
покрытие по allowlist имён тулов, дающих недоверенный контент.
"""
