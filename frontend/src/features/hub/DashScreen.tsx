...
import { useAsync, useBuilds, useHubRepositories, useInstances, useRunners } from "@/hooks";
import { FindingsTable } from "./PlaygroundScreen.tsx";
import { Dot, Panel, ago, useScreenCtx } from "./ui.tsx";
  ...
  return (
    <div className="screen">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16 }}>
        ...
        <Panel label="runners" className="elev pad"><div className="kv"><span>registered</span><span className="comment">{(runnersQ.data ?? []).length}</span></div></Panel>
      </div>
      <div className="box">
        <div className="thead" style={{ "--cols": ICOLS } as React.CSSProperties}><span></span><span>build</span><span>repository</span><span>events 24h</span><span>sandbox · updated</span></div>
        {instances.map((i) => (
          <div key={i.id} className="trow click" style={{ "--cols": ICOLS } as React.CSSProperties} onClick={() => navigate(`/instances/${i.id}`)}>
            ...
            <span className="muted small ellip">{i.sandboxExternalId ?? "no sandbox"}{i.sandboxStatus ? ` (${i.sandboxStatus})` : ""} · {ago(i.updatedAt)}</span>
          </div>
        ))}
        {instances.length === 0 && <div className="empty">{instancesQ.loading && instancesQ.data === undefined ? "loading…" : "no instances — connect a repository and a build."}</div>}
      </div>
      <div>
        <h2 style={{ marginBottom: 12 }}>latest findings <span className="muted small" style={{ fontWeight: 400 }}>· {findings.length}</span></h2>
        <FindingsTable rows={findings.slice(0, 12)} loading={findingsQ.loading && findingsQ.data === undefined} />
      </div>
    </div>
  );
}
