...
## Сабагенты (core/agents/subagents/)

Лид + делегирование через тул `task` (референс deer-flow, строго звезда глубины 1 — у детей тула `task` нет структурно). Wiring — в докстринге `core/agents/subagents/__init__.py`. Ключевое: `SubagentResult.try_set_terminal` — first-writer-wins (владение: executor → COMPLETED/FAILED, task-тул → TIMED_OUT/CANCELLED; отмена в executor НЕ терминализирует, а перебрасывает); причина капа — аддитивное поле `stop_reason`, не новый статус; квитанции (`receipts.py` — формат+парсер+верификатор+промпт в одном модуле), верификация цитат по снапшоту леджера цитирующего хода; лимиты — `SubagentLimitMiddleware` (вырезание task-вызовов + `finish_reason=stop`), admission — `SubagentCapacity` (FIFO, лимиты заморожены конструктором); события `task_*` — в custom-стрим. Ребёнку НИКОГДА не передавать `configurable`-ключи в конфиг. Тесты герметичны: `tests/conftest.py` глушит tracing-флаги (LangSmith-автотрейсинг сериализацией фейк-модели выпивает её итератор — реальный пойманный баг).
